# -*- coding: utf-8 -*-
"""
Created on Thu Nov  4 19:40:27 2021
...
if __name__ == '__main__':
    ...
    # contraction only
    import numpy as np
    wct, awct, freq = calc_cmc_epoched(list(np.array(EDC_Epochs)[:, :5*sampling_rate]), \
                                       list(np.array(C3)[:, :5*sampling_rate]), sampling_rate)
    ...
    wct, awct, freq = calc_cmc_epoched(list(np.array(EDC_Epochs)[:, 5*sampling_rate:]), \
                                       list(np.array(C3)[:, 5*sampling_rate:]), sampling_rate)
    plot(EDC_Epochs[0], C3[0], np.mean(wct, axis=0), awct, np.mean(freq, axis=0), "Relax only")