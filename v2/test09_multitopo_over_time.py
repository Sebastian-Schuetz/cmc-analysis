# -*- coding: utf-8 -*-
"""
Created on Mon Nov 15 10:23:33 2021
...
if __name__ == '__main__':
    ...
    for i in range(11):
        ...
        for chan in chansel:
            ...
            wct, freq = calc_cmc_epoched_welch(edc_e, C3, sampling_rate)
            wcts.append(np.mean(wct, axis=0))
    
        freqs = np.mean(freq, axis=0)
        freq_of_int = np.where(np.logical_and(freqs>=fmin, freqs<=fmax))[0]    
        
        plot_topomaps(np.array(wcts)[:, freq_of_int].T, freqs[freq_of_int], chansel, 5, 2, (0, 0.3), name="Shifted by " + str(i),
                      path = r"D:\Sebastian\Medizin\Doktorarbeit\Software\Github-Repos\cmc-analysis\plots\topo_maps\\" + str(i) + ".png")
        
        plot_coh_over_freq_welch(np.array(wcts)[chansel.index("C3"), :], freqs,
                                 "C3 - Shifed by: " + str(i), fmin, fmax, False, \
                                path = r"D:\Sebastian\Medizin\Doktorarbeit\Software\Github-Repos\cmc-analysis\plots\topo_maps\\" + "2d_" + str(i) + ".png")
