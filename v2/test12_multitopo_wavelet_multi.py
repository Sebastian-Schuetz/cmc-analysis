...
def calc_cmc(EDC_Epochs, EEG_Epochs, i, sampling_rate):
    ...
    plot_topomaps(np.array(wcts)[:, freq_of_int].T, freqs[freq_of_int], chansel, 5, 4, (0.3, 0.5), name="Shifted by " + str(i),
                  path = r"D:\Sebastian\Medizin\Doktorarbeit\Software\Github-Repos\cmc-analysis\plots\topo_maps_wave_laplace\\" +  str(i) + ".png")
    
    plot_coh_over_freq_welch(np.array(wcts)[chansel.index("C3"), :], freqs,
                              "C3 - Shifed by: " + str(i), fmin, fmax, False,\
                              path = r"D:\Sebastian\Medizin\Doktorarbeit\Software\Github-Repos\cmc-analysis\plots\topo_maps_wave_laplace\\" + "2d_" + str(i) + ".png")
...
if __name__ == '__main__':
    ...
    #Parallel(n_jobs=11)(delayed(calc_cmc)(EDC_Epochs, EEG_Epochs, i, sampling_rate) for i in range(11))
    
    for i in range(11):
        calc_cmc(EDC_Epochs, EEG_Epochs, i, sampling_rate)
